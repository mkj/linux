...
#define SHA_MSG_PADDING_LEN_SIZE_BYTES(shaType)   ((shaType == SHA_TYPE_SHA256 || shaType == SHA_TYPE_SHA1) ? \
                                                    SHA256_MSG_PADDING_LEN_SIZE_BYTES :                       \
                                                    SHA512_MSG_PADDING_LEN_SIZE_BYTES)
...
static int npcm_hash_walk_new_entry(struct crypto_hash_walk *walk)
{
	...
	if (walk->entrylen > walk->total) walk->entrylen = walk->total;
